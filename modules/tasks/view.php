...
if ($is_assigned_to_me) $can_update_task = true;  // Assignee can update
if ($is_assigned_by_me) $can_update_task = true;  // Assigner can update
if (Auth::hasRole('admin') || Auth::hasRole('hr')) $can_update_task = true;  // Admin/HR can update any
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_progress'])) {
    ...
    $status = $_POST['status'] ?? '';
    $comments = sanitizeText($_POST['comments'] ?? '', 2000, true);
...
$time_logs = null;
try {
    $time_stmt = $conn->prepare("SELECT SUM(hours) as total_hours FROM task_time_logs WHERE task_id = ?");
    if ($time_stmt) {
        $time_stmt->bind_param("i", $task_id);
        $time_stmt->execute();
        $time_total_hours = (float)($time_stmt->get_result()->fetch_assoc()['total_hours'] ?? 0);
    }
} catch (Throwable $e) {
    $time_total_hours = 0.0;
}
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="progress" style="height: 15px; width: 200px;">
                                            <div class="progress-bar <?php echo getProgressColorClass($task['progress']); ?>" 
                    ...
                    <form method="POST" action="add_comment.php" class="mb-4">
                        ...
                        <div class="d-flex justify-content-between">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="notify_team" id="notifyTeam" value="1">
        ...
        <div class="col-md-4">
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="d-flex justify-content-between align-items-center mb-2">
                            <div class="small text-muted">Total logged</div>
                            <div class="fw-bold"><?php echo number_format($time_total_hours, 2); ?>h</div>
                        </div>
                        <form method="POST" action="<?php echo $base_url; ?>/modules/tasks/log_time.php" class="mb-3">
                            <?php echo csrfField(); ?>
                            <input type="hidden" name="task_id" value="<?php echo (int)$task_id; ?>">
                            <div class="input-group input-group-sm mb-2">
                                <input type="number" name="hours" class="form-control" value="0.5" min="0.25" max="24" step="0.25" required>
                                <span class="input-group-text">hours</span>
                            </div>
                            <div class="input-group input-group-sm mb-2">
                                <input type="text" name="note" class="form-control" placeholder="Optional note...">
                                <button class="btn btn-outline-primary" type="submit">Log Time</button>
                            </div>
                        </form>
                        <small class="text-muted">Log time in 15-minute increments (0.25h).</small>
                        
                        <?php
                        // Fetch recent time logs
                        $logs_sql = "SELECT ttl.*, u.full_name as user_name
                                     FROM task_time_logs ttl
                                     JOIN users u ON ttl.user_id = u.id
                                     WHERE ttl.task_id = ?
                                     ORDER BY ttl.logged_at DESC, ttl.created_at DESC
                                     LIMIT 5";
                        $logs_stmt = $conn->prepare($logs_sql);
                        $logs_stmt->bind_param("i", $task_id);
                        $logs_stmt->execute();
                        ...
                        ?>
                        <div class="mt-3">
                            <small class="fw-semibold">Recent Time Logs:</small>
                                ...
                                <li class="list-group-item px-0 py-2">
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div>
                                            ...
                                            <strong><?php echo number_format($log['hours'], 2); ?>h</strong>
                                            <small class="text-muted ms-2"><?php echo htmlspecialchars($log['user_name']); ?></small>
                                        </div>
                                        <small class="text-muted"><?php echo formatDate($log['logged_at'], 'M d'); ?></small>
                                    </div>
                                    <?php if ($log['note']): ?>
                                    <small class="text-muted d-block mt-1"><i class="bi bi-chat-text me-1"></i><?php echo htmlspecialchars($log['note']); ?></small>
                                    <?php endif; ?>
                                </li>
                                <?php endwhile; ?>
                            </ul>
            ...
            <div class="card mb-4">
                ...
                <div class="card-body">
                    ...
                    $att_sql = "SELECT ta.id, ta.filename, ta.original_filename, ta.file_size, ta.uploaded_by, u.full_name as uploader_name
                                FROM task_attachments ta
                                LEFT JOIN users u ON ta.uploaded_by = u.id
                                WHERE ta.task_id = ?
                                ORDER BY ta.uploaded_at DESC";
                    $att_stmt = $conn->prepare($att_sql);
                    $att_stmt->bind_param("i", $task_id);
                    $att_stmt->execute();
                        ...
                        <div class="list-group-item d-flex justify-content-between align-items-center">
                            <div>
                                ...
                                <small class="text-muted d-block">
                                    <?php echo number_format(($att['file_size'] ?? 0) / 1024, 1); ?> KB • by <?php echo htmlspecialchars($att['uploader_name'] ?? 'Unknown'); ?>
                                </small>
                            </div>
                            <div class="btn-group">
                                <a href="<?php echo $base_url; ?>/modules/tasks/download_attachment.php?id=<?php echo (int)$att['id']; ?>" class="btn btn-sm btn-outline-primary" title="Download">
                                    <i class="bi bi-download"></i>
                                </a>
                                <?php if ($can_update_task): ?>
                                <form method="POST" action="<?php echo $base_url; ?>/modules/tasks/delete_attachment.php" onsubmit="return confirm('Delete this attachment?');" style="display:inline;">
                                    <?php echo csrfField(); ?>
                                    <input type="hidden" name="task_id" value="<?php echo (int)$task_id; ?>">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <?php
                    $related_sql = "SELECT t.id, t.title, t.status, t.progress, t.assigned_to, u.full_name as assignee_name
                                    FROM tasks t
                    ...
                    $related_stmt->bind_param("ii", $task['project_id'], $task_id);
                        ...
                        <a href="view.php?id=<?php echo $related['id']; ?>"
                           class="list-group-item list-group-item-action">
                            <div class="d-flex justify-content-between align-items-center mb-1">
                                <span class="small fw-medium"><?php echo htmlspecialchars($related['title']); ?></span>
                                <span class="badge bg-<?php echo $related['status'] == 'completed' ? 'success' : 'warning'; ?>">
                                    <?php echo $related['progress']; ?>%
                                </span>
                            </div>
                            <div class="small text-muted">
                                <i class="bi bi-person me-1"></i><?php echo htmlspecialchars($related['assignee_name'] ?? 'Unassigned'); ?>
                            </div>
                        </a>
                        <?php endwhile; ?>
                    </div>
            ...
            <form method="POST" action="add_comment.php">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" name="comment" rows="4" data-required="true"
...
<div class="modal fade" id="uploadModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="<?php echo $base_url; ?>/modules/tasks/upload_attachment.php" enctype="multipart/form-data">
                <?php echo csrfField(); ?>
                <input type="hidden" name="task_id" value="<?php echo (int)$task_id; ?>">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <input type="file" name="attachment" class="form-control" accept=".pdf,.doc,.docx,.xls,.xlsx,.jpg,.jpeg,.png" required>
                        <small class="text-muted">Max 10MB. Allowed: PDF, Office, JPG/PNG.</small>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
        ...
        <div class="modal-content">
            <form method="POST" action="delete.php">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-check mb-3">
                        <input class="form-check-input" type="checkbox" name="notify_assignee" id="notifyAssignee" value="1" checked>

...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        header('Location: view.php?id=' . $inquiry_id);
        exit();
    }
    
    $status = $_POST['status'] ?? '';
    $notes = sanitizeText($_POST['notes'] ?? '', 2000, true);
    ...
    if ($assigned_to !== null && $assigned_to !== '' && (!is_numeric($assigned_to) || (int)$assigned_to <= 0)) {
        $errors[] = 'Invalid assignee.';
    }
    if (!empty($errors)) {
    ...
    if ($update_stmt->execute()) {
        // Log activity to inquiry_activity table
        try {
            $uid = (int)($_SESSION['user_id'] ?? 0);
            if ($uid > 0) {
                $log_stmt = $conn->prepare("INSERT INTO inquiry_activity (inquiry_id, user_id, activity_type, notes) VALUES (?, ?, 'status_update', ?)");
                $log_notes = "Status updated to: " . $status;
                $log_stmt->bind_param("iis", $inquiry_id, $uid, $log_notes);
                $log_stmt->execute();
                $log_stmt->close();
            }
        } catch (Throwable $e) {
            error_log("Failed to log inquiry activity: " . $e->getMessage());
        }
...
// Load inquiry activity (call logs, etc.)
$activity_rows = [];
try {
    $act_stmt = $conn->prepare("SELECT ia.*, u.full_name
                                FROM inquiry_activity ia
                                JOIN users u ON ia.user_id = u.id
                                WHERE ia.inquiry_id = ?
                                ORDER BY ia.created_at DESC
                                LIMIT 20");
    if ($act_stmt) {
        $act_stmt->bind_param("i", $inquiry_id);
        $act_stmt->execute();
        $act_res = $act_stmt->get_result();
        while ($r = $act_res->fetch_assoc()) $activity_rows[] = $r;
    }
} catch (Throwable $e) {
    $activity_rows = [];
}
                ...
                <div class="card-body">
                    <div class="timeline small">
                        <?php if (!empty($activity_rows)): ?>
                                ...
                                <div class="timeline-item mb-3">
                                    ...
                                    <div class="timeline-content">
                                        <div class="text-muted">
                                            <?php echo formatDate($a['created_at'], 'M d, Y h:i A'); ?>
                                            <?php if (!empty($a['full_name'])): ?>
                                                · <?php echo htmlspecialchars($a['full_name']); ?>
                                            <?php endif; ?>
                                        </div>
                                        <div>
                                            <?php if (($a['activity_type'] ?? '') === 'call'): ?>
                                                <span class="badge bg-info text-dark">Call</span>
                                                <?php if (!empty($a['follow_up_date'])): ?>
                                                    <span class="badge bg-secondary">Follow-up: <?php echo htmlspecialchars($a['follow_up_date']); ?></span>
                                                <?php endif; ?>
                                                <div class="mt-1"><?php echo nl2br(htmlspecialchars($a['notes'] ?? '')); ?></div>
                                            <?php else: ?>
                                                <span class="badge bg-secondary"><?php echo htmlspecialchars($a['activity_type'] ?? 'activity'); ?></span>
                                                <div class="mt-1"><?php echo nl2br(htmlspecialchars($a['notes'] ?? '')); ?></div>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                            ...
                            <?php endforeach; ?>
                        <?php else: ?>
                            <div class="text-muted">No activity yet. Log a call to start the timeline.</div>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
            ...
            <form method="POST" action="send_reply.php">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" name="message" rows="8" data-required="true">
...
<div class="modal fade" id="callModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="log_call.php">
                <?php echo csrfField(); ?>
                <input type="hidden" name="inquiry_id" value="<?php echo (int)$inquiry_id; ?>">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            <input type="tel" class="form-control" value="<?php echo htmlspecialchars($inquiry['phone'] ?? ''); ?>" readonly>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" name="call_notes" rows="4" placeholder="Enter call notes..." required></textarea>
                    </div>

                    <div class="mb-3">
                        <label class="form-label">Next Follow-up Date (optional)</label>
                        <input type="date" name="follow_up_date" class="form-control" value="<?php echo date('Y-m-d', strtotime('+2 days')); ?>">
                    </div>

                    <div class="form-check">
                        <input class="form-check-input" type="checkbox" name="mark_contacted" id="markContactedCall" checked>
                        <label class="form-check-label" for="markContactedCall">
                            Mark inquiry as contacted
                        </label>
                    </div>
                </div>
            ...
            <form method="POST" action="convert.php">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="client_type" data-required="true">
            ...
            <form method="POST" action="close.php">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="close_reason" data-required="true">

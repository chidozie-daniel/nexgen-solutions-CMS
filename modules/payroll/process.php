...
// Check login status and require HR or admin role
Auth::requireRole(['hr', 'admin']);

$page_title = 'Process Payroll';
$conn = getDBConnection();
$current_month = date('Y-m');
$selected_month = $_GET['month'] ?? $current_month;
if (!isValidMonth($selected_month)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && isset($_POST['id'])) {
    ...
    $allowed_actions = ['approve', 'pay', 'reject'];
    if ($salary_id <= 0 || !in_array($action, $allowed_actions, true)) {
        setFlash('danger', 'Invalid payroll action.');
        header('Location: ' . Auth::getBasePath() . '/modules/payroll/process.php?month=' . urlencode($selected_month));
        exit();
    }
    ...
    switch ($action) {
        case 'approve':
            ...
            $stmt = dbPrepare($conn, $sql, 'payroll approve');
            if (!$stmt) {
                setFlash('danger', 'Could not approve salary. Please try again.');
                break;
            }
            $stmt->bind_param("i", $salary_id);
            if (dbExecute($stmt, 'payroll approve')) {
        ...
        case 'pay':
            ...
            $stmt = dbPrepare($conn, $sql, 'payroll pay');
            if ($stmt) {
                $stmt->bind_param("si", $payment_date, $salary_id);
                if (dbExecute($stmt, 'payroll pay')) {
                    setFlash('success', 'Salary marked as paid.');
                } else {
                    setFlash('danger', 'Could not mark salary as paid. Please try again.');
                }
                $stmt->close();
            } else {
                setFlash('danger', 'Could not mark salary as paid. Please try again.');
        ...
        case 'reject':
            ...
            $stmt = dbPrepare($conn, $sql, 'payroll reject');
            if ($stmt) {
                $stmt->bind_param("i", $salary_id);
                if (dbExecute($stmt, 'payroll reject')) {
                    setFlash('warning', 'Salary returned for review.');
                } else {
                    setFlash('danger', 'Could not update salary status. Please try again.');
                }
                $stmt->close();
            } else {
                setFlash('danger', 'Could not update salary status. Please try again.');
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['bulk_action'])) {
    ...
    $allowed_bulk = ['approve_all', 'pay_all', 'export_payslips'];
    if (!in_array($action, $allowed_bulk, true)) {
        setFlash('danger', 'Invalid bulk action.');
        header('Location: ' . Auth::getBasePath() . '/modules/payroll/process.php?month=' . urlencode($selected_month));
        exit();
    }
    ...
    if (!empty($selected_ids)) {
        ...
        $selected_ids = array_filter(array_map(function($id) { return (int)$id; }, $selected_ids), function ($id) {
            return $id > 0;
        });
        if (empty($selected_ids)) {
            setFlash('danger', 'No valid salaries selected.');
            header('Location: ' . Auth::getBasePath() . '/modules/payroll/process.php?month=' . urlencode($selected_month));
            exit();
        }
        ...
        switch ($action) {
            case 'approve_all':
                ...
                $stmt = dbPrepare($conn, $sql, 'payroll bulk approve');
                if ($stmt) {
                    $types = str_repeat('i', count($selected_ids));
                    $stmt->bind_param($types, ...$selected_ids);
                    if (dbExecute($stmt, 'payroll bulk approve')) {
                        setFlash('success', 'Selected salaries approved.');
                    } else {
                        setFlash('danger', 'Could not approve selected salaries. Please try again.');
                    }
                    $stmt->close();
                } else {
                    setFlash('danger', 'Could not approve selected salaries. Please try again.');
            ...
            case 'pay_all':
                ...
                $stmt = dbPrepare($conn, $sql, 'payroll bulk pay');
                if ($stmt) {
                    $types = 's' . str_repeat('i', count($selected_ids));
                    $stmt->bind_param($types, $payment_date, ...$selected_ids);
                    if (dbExecute($stmt, 'payroll bulk pay')) {
                        setFlash('success', 'Selected salaries marked as paid.');
                    } else {
                        setFlash('danger', 'Could not mark selected salaries as paid. Please try again.');
                    }
                    $stmt->close();
                } else {
                    setFlash('danger', 'Could not mark selected salaries as paid. Please try again.');

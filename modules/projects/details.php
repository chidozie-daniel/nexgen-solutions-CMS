                ...
                <div class="card-body text-center">
                    <div class="mb-3">
                        <div class="progress" style="height: 20px;">
                            <div class="progress-bar <?php echo getProgressColorClass($project_progress); ?>" 
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            <button type="button" class="btn btn-sm btn-link text-primary p-0" 
                                                    data-bs-toggle="modal" data-bs-target="#updateTaskProgressModal<?php echo $task['id']; ?>"
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select" name="assigned_to" data-required="true">
            ...
            <form method="POST" action="add_member.php">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="user_id" data-required="true">
                            <option value="">Choose team member...</option>
                            <?php
                            // Get role-appropriate team members based on who is adding
                            $creator_role = $_SESSION['role'];
                            if ($creator_role === 'admin' || $creator_role === 'hr') {
                                // Admin/HR: Show Project Leaders and Employees
                                $all_employees_sql = "SELECT id, full_name, employee_id, department, role
                                                    FROM users
                                                    WHERE status = 'active'
                                ...
                                // Project Leader: Show Employees only
                                $all_employees_sql = "SELECT id, full_name, employee_id, department, role
                                                    FROM users
                                                    WHERE status = 'active'
                                                    AND role = 'employee'
                        ...
                        <small class="text-muted">
                            <?php if ($creator_role === 'admin' || $creator_role === 'hr'): ?>
                                Select Project Leaders or Employees to add to this project.
                            <?php else: ?>
...
<!-- Update Task Progress Modal (for each task) -->
<?php
// Reset tasks result to add modals
$tasks_stmt = $conn->prepare($tasks_sql);
$tasks_stmt->bind_param("i", $project_id);
$tasks_stmt->execute();
$tasks_result = $tasks_stmt->get_result();
...
?>
<div class="modal fade" id="updateTaskProgressModal<?php echo $task['id']; ?>" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="<?php echo $base_url; ?>/modules/tasks/update_progress.php">
                <?php echo csrfField(); ?>
                <input type="hidden" name="task_id" value="<?php echo $task['id']; ?>">
                <input type="hidden" name="project_id" value="<?php echo $project['id']; ?>">
                <div class="modal-header">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="range" class="form-range" min="0" max="100" step="5"
                               id="taskProgress<?php echo $task['id']; ?>"
                               name="progress" value="<?php echo $task['progress']; ?>"
                               oninput="document.getElementById('progressValue<?php echo $task['id']; ?>').innerText = this.value + '%'">
                        <div class="d-flex justify-content-between">
                            <small>0%</small>
                            <span id="progressValue<?php echo $task['id']; ?>" class="fw-bold">
                                <?php echo $task['progress']; ?>%
                            </span>
                            <small>100%</small>
                        </div>
